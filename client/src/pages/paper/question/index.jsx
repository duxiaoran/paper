...
export default () => {
  ...
  const [answer, setAnswer] = useState({value:{}, openid: userInfo.openid, status: 1, paper_id: router.params.id});
  ...
  useDidShow(async ()=>{
    try {
      ...
      const answerRes = await Taro.cloud.callFunction({name: "answer",data: {action: 'get',data: {paper_id: router.params.id, openid: userInfo.openid} } });
  ...
  const submit = async () => {
    if(!userInfo.avatarUrl){
      ...
      const userInfoData = await Taro.cloud.callFunction({name: "user",data: {action: 'get',data: {openid: userInfo.openid} } });
  ...
  return (
    <View className={styles.paper_question}>
      <View className={styles.paper_question_title}>
        ...
        {paperInfo.pass?.includes(userInfo.openid) && <Text className={styles.paper_question_title_status}>已交卷</Text>}
      </View>
